...
private let backgroundCardsTopMargin: CGFloat = 9.0
...
public class KolodaView: UIView, DraggableCardDelegate {
    ...
    public func frameForCardAtIndex(index: UInt) -> CGRect {
        ...
        let yOffset = (previousCardFrame.origin.y - backgroundCardsTopMargin) * multiplier
    ...
    public func refreshFirstCard() {
      if visibleCards.count > 0 && animating {
        if let  firstCardView = visibleCards.first {
          firstCardView.resetViewPositionAndTransformations()
        }
      }
    }
