...
class GGParentVC: UIViewController {
  ...
  var viewNoInternetConnection: NoInternetConnectionView?
  
  var request: Request?
  
  var lblTitle: UILabel! = UILabel()
  var strTitle: String! = ""
  var fontTitle: UIFont! = getJosefinSansBoldWith(18)
...
//    }
  ...
  // MARK: - Public Method
  func setTitleForVC(title: String, font: UIFont = getJosefinSansBoldWith(18)) {
    strTitle = title
    fontTitle = font
  ...
  func setImageNavibar(image: UIImage? = UIImage(named: "image_navibar")!.resizableImageWithCapInsets(UIEdgeInsetsMake(0, 0, 0, 0), resizingMode: .Stretch)) {
  ...
  func setNumberBookingForNotificationRightButton(number: Int = 0) {
    if let imgBooking = UIImage(named: "ic_navi_favorite") {
      ...
      lblNumber.font = getProximaNovaBoldWith(10)
...
//      self.navigationItem.leftBarButtonItems = []
      ...
      let font = getProximaNovaRegularWith(14)
...
//  }
  ...
  private func createBarButton(image: UIImage?, title: String = "", actionString: String) -> UIBarButtonItem {
    ...
    if let image = image {
      ...
      let font = getJosefinSansBoldWith(18)
      let dic = [NSFontAttributeName: font, NSForegroundColorAttributeName : UIColor.whiteColor()]
...
// MARK: - NoInternetConnectionDelegate method
extension GGParentVC: NoInternetConnectionDelegate {
  func didTouchTryAgain(isTouch: Bool) {
    if isTouch {
      tapToTryAgainConnection()
    }
  }
}
